"""Final leakage-safe LC25000 model evaluation."""
...
from config import LC25000_LEAKAGE_SAFE_SPLIT_FILE, LEAKAGE_SAFE_EVAL_DIR, RANDOM_STATE, RESULTS_DIR
...
def save_logistic_outputs(dataset, model_name, y_test, y_pred, y_prob):
    LEAKAGE_SAFE_EVAL_DIR.mkdir(parents=True, exist_ok=True)
    ...
    pd.DataFrame(report).transpose().to_csv(LEAKAGE_SAFE_EVAL_DIR / f"LC25000_{model_name}_classification_report.csv")
    ...
    df.to_csv(LEAKAGE_SAFE_EVAL_DIR / f"LC25000_{model_name}_predictions.csv", index=False)

def run_logistic(dataset, X_train, y_train, X_val, y_val, X_test, y_test):
    model_name = "LogisticRegression_Downsampled_LeakageSafe"
    ...
    return {"dataset": dataset.name, "split_type": "leakage_safe_resized_image_hash_grouped_split", "model": model_name, "accuracy": round(float(accuracy), 6), "precision": round(float(precision), 6), "recall": round(float(recall), 6), "weighted_f1": round(float(weighted_f1), 6), "macro_f1": round(float(macro_f1), 6), "balanced_accuracy": round(float(balanced_accuracy), 6), "roc_auc": round(float(roc_auc), 6) if not np.isnan(roc_auc) else np.nan, "epochs": "n/a", "train_samples": len(y_train_full), "validation_samples": len(y_val), "test_samples": len(y_test), "notes": "Downsampled 32x32 grayscale Logistic Regression baseline on leakage-safe split"}, cm

def build_model(model_key, num_classes):
    if model_key == "simplecnn": model = SimpleCNN(num_classes); model.model_name = "SimpleCNN_LeakageSafe"; return model
    if model_key == "resnet18": model = TransferCNN(num_classes, "staged_finetune"); model.model_name = "TransferCNN_ResNet18_LeakageSafe"; return model
    if model_key == "headonly": model = TransferCNN(num_classes, "head_only"); model.model_name = "TransferCNN_ResNet18_HeadOnly_LeakageSafe"; return model
    if model_key == "densenet121": model = TransferDenseNet121(num_classes, "staged_finetune"); model.model_name = "TransferCNN_DenseNet121_LeakageSafe"; return model
    if model_key == "efficientnetb0": model = TransferEfficientNetB0(num_classes, "staged_finetune"); model.model_name = "TransferCNN_EfficientNetB0_LeakageSafe"; return model
    raise ValueError(f"Unknown model key: {model_key}")

def copy_deep_model_outputs_to_leakage_safe_dir(dataset, model):
    LEAKAGE_SAFE_EVAL_DIR.mkdir(parents=True, exist_ok=True)
    for suffix in ["predictions", "classification_report"]:
        src = RESULTS_DIR / f"{dataset.name}_{model.model_name}_{suffix}.csv"
        dst = LEAKAGE_SAFE_EVAL_DIR / f"LC25000_{model.model_name}_{suffix}.csv"
...
def run_deep_model(model_key, dataset, X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    save_confusion_matrix(cm, f"{dataset.name}_{model.model_name}"); save_training_curves(history, f"{dataset.name}_leakage_safe", model.model_name); copy_deep_model_outputs_to_leakage_safe_dir(dataset, model)
    return {"dataset": dataset.name, "split_type": "leakage_safe_resized_image_hash_grouped_split", "model": model.model_name, "accuracy": round(float(accuracy), 6), "precision": round(float(precision), 6), "recall": round(float(recall), 6), "weighted_f1": round(float(weighted_f1), 6), "macro_f1": round(float(macro_f1), 6), "balanced_accuracy": round(float(balanced_accuracy), 6), "roc_auc": round(float(roc_auc), 6) if not np.isnan(roc_auc) else np.nan, "epochs": history["epochs_ran"], "train_samples": len(y_train), "validation_samples": len(y_val), "test_samples": len(y_test), "notes": "Deep model evaluated on leakage-safe split"}, cm

def update_all_model_summary(result_row):
    path = LEAKAGE_SAFE_EVAL_DIR / "lc25000_leakage_safe_all_model_summary.csv"
    ...
    df.to_csv(path, index=False); print(f"Updated combined leakage-safe summary: {path}")

def parse_args():
    p = argparse.ArgumentParser(description="Run leakage-safe LC25000 model evaluation."); p.add_argument("--model", required=True, choices=["logistic", "simplecnn", "resnet18", "headonly", "densenet121", "efficientnetb0"]); return p.parse_args()

def main():
    args = parse_args(); start = time.perf_counter(); set_seed(); LEAKAGE_SAFE_EVAL_DIR.mkdir(parents=True, exist_ok=True)
    dataset = [d for d in load_datasets() if d.name == "LC25000"][0]
    train_idx, val_idx, test_idx = load_leakage_safe_split()
    ...
    result_path = LEAKAGE_SAFE_EVAL_DIR / f"lc25000_{args.model}_leakage_safe_metrics.csv"; pd.DataFrame([result_row]).to_csv(result_path, index=False); update_all_model_summary(result_row)
    print("\nLeakage-safe result:"); [print(f"{k}: {v}") for k, v in result_row.items()]; print("Confusion Matrix:\n", cm); print(f"\nSaved metrics to: {result_path}"); print(f"Total time: {(time.perf_counter() - start) / 60:.4f} minutes")
